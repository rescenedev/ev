...
**Visual (inline image) preview** (`lib/visual.sh`, wired into `_ev_preview` in `ev`): `Ctrl-V` toggles the preview between text extraction and a Finder-style inline image of the document's first page. Pipeline: `libexec/ev-qlthumb.swift` (a `QLThumbnailGenerator` CLI, compiled with `swiftc -O` on first use and cached at `${XDG_CACHE_HOME:-~/.cache}/ev/ev-qlthumb`) renders any QuickLook-supported file (hwpx/docx/pptx/xlsx/pdf/images) to a PNG, then `chafa -f kitty` emits it via the kitty graphics protocol. Thumbnails are cached in `$EV_STATE/thumbs/` keyed by path+mtime. Requires a kitty-graphics terminal (Ghostty/kitty/WezTerm) + `chafa`; hwpx needs the Alhangeul QuickLook extension. Any missing piece falls back to the text preview with a one-line hint. `qlmanage -t` is **not** used — it hangs on modern QuickLook App Extensions like hwpx.
